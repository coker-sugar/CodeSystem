	...
	<div class="wrapper">
		...
		<div class="modal" v-show="modal">
			<el-form :model="user" status-icon :rules="rules" ref="userForm"  @keyup.enter.native="login">
				...
				<el-form-item>
					...
					<el-button type="primary" class="btn-login" @click="login" v-throttle>登录</el-button>
				</el-form-item>
			</el-form>
		</div>
		...
		<div v-show="register">
        	<el-form ref="registerForm" :model="form" :rules="RegisterRules" label width=" 80px" class="register-box">
        	    <h3 class="register-title">欢迎新用户！</h3>
        	    <el-form-item label="账号" prop="account">
        	        <el-input type="text" placeholder=" 请输入账号" prefix-icon="el-icon-user" v-model="form.account"/>
        	    </el-form-item>
        	    ...
        	    <el-form-item label="确认密码" prop="checkPass">
        	        <el-input :type="psw1?'password':'text'" placeholder=" 请确认密码" prefix-icon="el-icon-lock" v-model="form.checkPass" >
						<i
      				    slot="suffix"
      				    class="icon-style"
      				    :class="psw2 ? 'el-icon-view': 'el-icon-minus'"
      				    autocomplete="auto"
      				    @click="psw2 = !psw2"
      				  	/>
					</el-input>
...
methods: {
	...
	// 			 this.$axios.get('http://127.0.0.1:4523/m1/3171021-0-default/login',{  // 管理员13946473676  123
                    ...
                    await Request.Submit({
                        ...
                        .then(response => {
                            ...
                            if(response.status === 200){
                                ...
                                this.modal = !this.modal
                                this.register = !this.register
                                this.$message.success(this.form.account+"注册成功")
								this.form = {}
                            }else{
                                console.log(Boolean(response==1))
                                this.$message.error(this.form.account+"已被注册")
                            }
                        })
                // } else {
